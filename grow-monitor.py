...
class MainView(View):
    ...
    def render_channel(self, channel):
        ...
        # TODO: replace numbezr text with graphic
...
# Basic logging so failures are visible in journalctl
logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')

# Email setup (lazy connect + resilient to DNS/network failures)
# NOTE: credentials are currently in-file; consider moving to environment vars
from_mail = os.getenv('GROW_FROM_MAIL', 'jasonvegapi@gmail.com')
sms_recipient = os.getenv('GROW_SMS_RECIPIENT', 'jasonvegapi@gmail.com')

_smtp = None
_smtp_last_attempt = 0
# seconds between reconnection attempts (can be tuned with env)
_smtp_attempt_interval = int(os.getenv('GROW_SMTP_RETRY_SECS', '60'))

SMTP_HOST = os.getenv('GROW_SMTP_HOST', 'smtp.gmail.com')
SMTP_PORT = int(os.getenv('GROW_SMTP_PORT', '587'))
SMTP_USER = os.getenv('GROW_SMTP_USER', 'jasonvegapi@gmail.com')
SMTP_PASS = os.getenv('GROW_SMTP_PASS', 'emur hbrf hygw mgdp')

def _get_smtp():
    """Return a connected SMTP object or None.

    This function will only attempt to reconnect at most once per
    `_smtp_attempt_interval` seconds to avoid repeated DNS failures
    bringing down the service.
    """
    global _smtp, _smtp_last_attempt

    if _smtp is not None:
        return _smtp

    now = time.time()
    if now - _smtp_last_attempt < _smtp_attempt_interval:
        return None

    _smtp_last_attempt = now
    ...
    try:
        ...
        _smtp = s
        logging.info("SMTP connection established to %s", SMTP_HOST)
        return _smtp
    except (smtplib.SMTPException, socket.gaierror, OSError) as e:
        logging.warning("SMTP connection failed: %s", e)
        _smtp = None
        return None

def sendMessage(body, to=sms_recipient):
    """Send a message (best-effort). Returns True if sent, False otherwise."""
    body = str(body)
    s = _get_smtp()
    if s is None:
        logging.warning("sendMessage: no SMTP connection available; skipping message: %s", body)
        return False
    ...
    try:
        ...
        logging.warning("sendMessage failed, dropping SMTP connection: %s", e)
        try:
            s.quit()
        except Exception:
            pass
        # reset cached connection so future calls will attempt reconnect
        global _smtp
        _smtp = None
        return False
...
"""
    ...
    def update(self):
        ...
        if sat < self.water_level:
            if self.water():
                logging.info(
                    f"Watering Channel: {self.channel} - rate {self.pump_speed:.2f} for {self.pump_time:.2f}sec"
                )
                sendMessage(f"Auto-watering triggered on Channel {self.channel}")
        if sat < self.warn_level:
            if not self.alarm:
                logging.warning(
                    f"Alarm on Channel: {self.channel} - saturation is {sat * 100:.2f}% (warn level {self.warn_level * 100:.2f}%)"
                )
                sendMessage(f"Warning: Moisture low on Channel {self.channel}")
            self.alarm = True
        else:
            self.alarm = False
...
""".format(
    ...
    while True:
        try:
            ...
            # Sleep a bit to avoid tight exception loop
            time.sleep(5)
